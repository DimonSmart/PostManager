...
### 8.0 UI конвенции CRUD
- Для всех сущностей CRUD страницы раздельные:
  - List: `/entities`
  - Create: `/entities/new`
  - Edit: `/entities/{id}`
- На странице списка нет формы создания: только таблица/грид, действия Edit/Delete и кнопка Create.
- Удаление выполняется только с подтверждением через диалог.
- Create/Edit используют общий компонент формы.
- После успешного Create выполняется переход на список с `selectedId`, список подсвечивает созданную строку, прокручивает к ней и опционально показывает уведомление "Created".
- Основная UI-библиотека проекта: MudBlazor (MudTable/MudDataGrid, MudForm/MudTextField, MudSnackbar, MudDialog).
